...
<html>
  ...
  <body>
    ...
    <c:if test="${!empty list}">
          <c:forEach items="${list}" var="hkmxLog">
          
          <tr>
            <td>${hkmxLog.hksj}</td>
            <td>${hkmxLog.hkje}</td>
            <td><a href="hkmxLog!detail.action?logno=${hkmxLog.logNo}">详细</a>&nbsp;<a href="hkmxLog!delete.action?logno=${hkmxLog.id}" onClick="delete(${sxkh.id})">删除</a></td>
          </tr>
          </c:forEach>
	</c:if>
